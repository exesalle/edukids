...
import {Badge, Button, DatePicker, DatePickerProps, Input, Modal, Select, Space, Table, Tag, TimePicker} from 'antd';
...
const AdminEvents:React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: string, item: IEventsData) => {
        return (
          <>
            1 место: <Tag color="purple">{item.prize.first}</Tag><br />
            2 место: <Tag color="purple">{item.prize.second}</Tag><br />
            3 место: <Tag color="purple"> {item.prize.third}</Tag>
          </>);
      }
    },
    {
  ...
  const showModal = (el:IEventsData) => {
    ...
    setEditData({
      ...
      time: el.time,
      prize: {
        first: el.prize.first,
        second: el.prize.second,
        third: el.prize.third,
      }
    });
  };
  ...
  const handleOnChangeFirst = (value:string) => {
    setEditData({...editData,  prize:{...editData.prize, first: value}});
  };
  const handleOnChangeSecond = (value:string) => {
    setEditData({...editData,  prize:{...editData.prize, second: value}});
  };

  const handleOnChangeThird = (value:string) => {
    setEditData({...editData,  prize:{...editData.prize, third: value}});
  };
  ...
  const handleAddEvent = () => {
    dispatch(addEvent({
      ...
      prize: {
        first: eventsData.prize.first,
        second: eventsData.prize.second,
        third: eventsData.prize.third,
      }
    }));
      ...
      <div className="main-screen">
        ...
        <Modal title="Редактировать" open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
          ...
          <div className="input-course">
            ...
            <DatePicker defaultValue={dayjs(editData.date.toString(), 'DD-MM-YYYY')} style={{ width: 130 }} format="DD-MM-YYYY" onChange={onChangeDate} />

            <TimePicker
              defaultValue={dayjs(editData.time.toString(), 'HH:mm')}
              style={{ width: 130 }}
              minuteStep={5}
              hourStep={1}
              format={format}
          ...
          <Select defaultValue={editData.prize.first}  style={{ width: 230 }} onSelect={(value) => handleOnChangeFirst(value)} >
            {users.map((el, index) =>
              <Select.Option value={el.name} key={index} >{el.name}</Select.Option >
            )}
          </Select><br/>
          <Tag color="purple">2 место:</Tag>
          <Select defaultValue={editData.prize.second}  style={{ width: 230 }} onSelect={(value) => handleOnChangeSecond(value)} >
            {users.map((el, index) =>
              <Select.Option value={el.name} key={index} >{el.name}</Select.Option >
            )}
          </Select><br/>
          <Tag color="purple">3 место:</Tag>
          <Select defaultValue={editData.prize.third}  style={{ width: 230 }} onSelect={(value) => handleOnChangeThird(value)} >
            {users.map((el, index) =>
              <Select.Option value={el.name} key={index} >{el.name}</Select.Option >
            )}
          </Select>
